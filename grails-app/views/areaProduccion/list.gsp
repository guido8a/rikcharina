<div class="container" style="width: 100%">
    <div class="panel-info" style="padding: 3px; margin-top: 2px">
        <div class="btn-toolbar toolbar">
            <div class="btn-group">
                <a href="#" class="btn btn-sm btn-success" id="btnAgregarArea">
...
<script type="text/javascript">
    ...
    function cargarTablaAreas(){
        $.ajax({
            type: 'POST',
            url:'${createLink(controller: 'areaProduccion', action: 'tablaAreas_ajax')}',
            data:{finca: '${finca}'},
    ...
    function createEditArea(id) {
        ...
        $.ajax({
            type    : "POST",
            url     : "${createLink(controller:'areaProduccion', action:'form_ajax')}",
            data    : data,
            success : function (msg) {
                var b = bootbox.dialog({
                    id      : "dlgCreateEdit",
                    title   : title + " Área de producción",
                    message : msg,
                    buttons : {
                        cancelar : {
                            label     : "Cancelar",
    ...
    function borrarArea(id){
        bootbox.confirm({
            message: "<i class='fa fa-3x fa-exclamation-triangle text-danger'></i> <strong style='font-size: 14px'>  Está seguro de eliminar esta área de producción? </strong>",
            buttons: {
                confirm: {
                    label: 'Borrar',
                    className: 'btn-success'
            ...
            callback: function (result) {
                if(result){
                    $.ajax({
                        type:'POST',
                        url:'${createLink(controller: 'areaProduccion', action: 'borrarArea_ajax')}',
                        data:{
                            id: id
                        },
                        success:function(msg){
