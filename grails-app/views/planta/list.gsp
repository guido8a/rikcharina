...
<div class="panel panel-primary col-md-6" style="margin-top: 10px; text-align: center">
    ...
    <div class="panel-info" style="padding: 3px; margin-top: 2px">
        <div class="btn-toolbar toolbar">
            <div class="btn-group">
                <a href="#" class="btn btn-sm btn-success" id="btnAgregarPlanta">
                    <i class="fa fa-plus"></i> Agregar planta
                </a>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function cargarTablaPlanta(){
        $.ajax({
            type: 'POST',
            url:'${createLink(controller: 'planta', action: 'tablaPlantas_ajax')}',
            data:{},
            success: function(msg){
                $("#tablaPlanta").html(msg)
            }
        });
    }

    function createEditPlanta(id) {
        var title = id ? "Editar" : "Crear";
        var data = id ? {id : id} : {};
        $.ajax({
            type    : "POST",
            url     : "${createLink(controller:'planta', action:'form_ajax')}",
            data    : data,
            success : function (msg) {
    ...
    function borrarPlanta(id){
        bootbox.confirm({
            message: "<i class='fa fa-3x fa-exclamation-triangle text-danger'></i> <strong style='font-size: 14px'>  Está seguro de eliminar esta planta? </strong>",
            buttons: {
                confirm: {
                    label: 'Borrar',
                    className: 'btn-success'
            ...
            callback: function (result) {
                if(result){
                    $.ajax({
                        type:'POST',
                        url:'${createLink(controller: 'planta', action: 'borrarPlanta_ajax')}',
                        data:{
                            id: id
                        },
                        success:function(msg){
